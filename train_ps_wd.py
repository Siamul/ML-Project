...
# training options
parser.add_argument('--save_dir', default='./experiments_nodecoder',
...
class TextEncoder(nn.Module):
    def __init__(self, enc_type, hidden_dim, output_dim, device, n_layers=5):
        super().__init__()
        word_embedding_model =  models.Transformer(enc_type)
        pooling_model = models.Pooling(word_embedding_model.get_word_embedding_dimension())
        dense_model = models.Dense(in_features=pooling_model.get_sentence_embedding_dimension(), out_features=hidden_dim, activation_function=nn.ReLU())
        self.encoder = net.SentenceTransformerv2(modules=[word_embedding_model, pooling_model, dense_model])
        self.encoder.train()
    ...
    def forward(self, input):
        embedding = self.encoder.encode_with_gradients(input)
        intermediate = self.input_layer(embedding)
...
text_encoder = TextEncoder('sentence-transformers/all-mpnet-base-v2', 1024, 1024, device, n_layers=10)           
